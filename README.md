...
直播源转推到 YouTube 时，系统必须优先使用“当前这一场直播”的本地录播作为兜底源，而不是默认使用媒体库旧文件或其他主播文件。
...
抖音 / B站 / 快手直播源
    -> VPS 拉取直播直链
    -> FFmpeg 推到 YouTube
    -> 同一条输入同步写入 VPS 本地录播文件
```

当直播源异常时，YouTube 数据源按以下顺序切换：

```text
1. 当前直播实时直链
2. 当前正在录制的临时片段快照
3. 当前这一场已保存的自动录播文件
4. task_<taskId>_latest.ts 兼容录播链接
5. 无可用录播时，才进入断流/重试状态
```

也就是说，源直播关播后，YouTube 不应直接无数据退出；系统应先播放 VPS 上当前任务、当前场次已经录到的内容，并循环维持目标直播间。只有这场直播没有成功录到可用文件时，YouTube 才存在无兜底源的断播风险。

当前自动录播文件位于执行 VPS：

```text
/root/restream_uploads/录播_<YYYYMMDD_HHMMSS>_<频道名>_task<taskId>.ts
```

录播文件用于兜底，不等同于用户手动上传的媒体库文件。手动文件只应作为纯文件转播或显式选择的素材管理入口。
