...
const healthSource = [
  fs.readFileSync(path.join(root, 'services/ffmpeg-args.js'), 'utf8'),
  fs.readFileSync(path.join(root, 'services/task-state.js'), 'utf8'),
  fs.readFileSync(path.join(root, 'services/task-manager.js'), 'utf8'),
].join('\n');
...
assertContains(healthSource, '"bg_pid"', '健康检测状态文件记录后台兜底 PID');
assertContains(healthSource, 'bg_fallback_alive', '健康检测识别后台兜底存活状态');
assertContains(healthSource, 'wrapper process exited but fallback is still pushing', '后台兜底仍在推流时不误触发自动重启');
assertContains(healthSource, 'return restart(null', '主进程死亡自动重启前走统一清理分支');
assertContains(healthSource, 'startTask/preCleanup', '任务启动前按任务 ID 清理旧远端会话');
...
assertContains(healthSource, 'const isBlocked', '验证码封锁状态不再通过日志关键字误判');
