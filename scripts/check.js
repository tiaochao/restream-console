...
assertContains(generatedScript, '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 10', '直播输入短线自动重连');
...
assertContains(healthSource, "tail -n ${HEALTH_LOG_TAIL_LINES} ${task.log_file}", '健康检测扩大日志尾部扫描范围');
assertContains(healthSource, 'const expectsRtmp1935 = /^rtmp:\\/\\//i.test', '仅对 rtmp:// 检查 1935 连接');
...
assertContains(healthSource, 'will reconnect.*error=end of file', '源直链 EOF 按直链过期处理');
assertContains(healthSource, 'const targetErrorPattern =', 'RTMP 错误按单行定位，避免日志尾部旧错误误判');
assertContains(healthSource, 'const hasHealthyFrameAfterErrors', '健康检查用最新 frame 进度压制旧错误误判');
assertContains(healthSource, 'lastTargetErrorIndex > Math.max(lastGoodStreamIndex, lastSourceErrorIndex, lastFrameIndex)', '只有目标端错误发生在最新有效流之后才判定 RTMP 断开');
...
assertContains(healthSource, 'verify failed|verification required', '验证码判断避免误伤 URL 参数');
